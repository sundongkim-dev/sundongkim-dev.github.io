...
> Info<sub>A</sub>(D) = sum(abs(D<sub>j</sub>) / abs(D) * Info(D<sub>j</sub>))

A라는 Attribute로 D를 v개의 partition으로 나누었다고 한다면 sum은 j=1부터 v까지의 합이다.
...
결과적으로 분기 후의 엔트로피가 제일 작은 값을 택한 것이 information gain을 제일 크게 만드는 것이 된다.
...
공식을 보면 어딘가 익숙하다. Normalization이 떠오르지 않는가? Gain ratio는 information에 대해 normalization을 해준 결과이다. Normalization으로 input data의 편향을 해소해준 것이다.

결과적으로 information gain과 마찬가지로 gain Ratio가 가장 큰 attribute가 선정된다.

이 역시도 만능이 아니다. 불균형이 심한 attribute를 상대적으로 선택하기 쉬워진다. ~~왜??~~
...
결과적으로 가장 'reduction in impurity'가 큰 attribute를 선택한다. 결국 가장 작은 gini<sub>A</sub>(D)값을 선택해야 한다.
...
오늘 우리가 학습 decision tree에서의 overfitting은 모든 튜플이 각각 하나의 브랜치를 갖고 있는 경우이다.
...
1. 상대적으로 학습 속도가 다른 classification method들에 비해 빠르다.
2. 간단하고 쉽게 이해할 수 있는 classification rule로 변환 가능하다.
...
4. 다른 classification 방법과 정확도를 비교할 수 있다.
