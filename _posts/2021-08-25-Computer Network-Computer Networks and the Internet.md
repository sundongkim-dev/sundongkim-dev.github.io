...
이 때, 전달 테이블을 설정하기 위해 이용되는 여러 특별한 라우팅 프로토콜이 있다. 예를 들어, 라우팅 프로토콜은 각 라우터로부터 각 목적지까지의 최단 경로를 결정하고 라우터에 전달 테이블을 설정하는 데 이 최단 경로 결과를 이용한다.
...
접속 ISP들이 서로 연결되어야만 함 = 모든 종단 시스템이 서로에게 패킷을 보내려고
직접 서로 다른 ISP와 연결 = 너무 많은 비용 발생, 개별적인 통신링크를 유지해야 하기 때문
①모든 접속 ISP들을 하나의 global transit ISP와 연결 = 비용↑
  ▷과금(고객=접속ISP, 제공자=글로벌ISP) ▷ 네트워크 구조1
②수십만 개의 접속 ISP와 다중의 글로벌 ISP = 가격,서비스 비교 경쟁 ▷ 네트워크 구조2
  But, 글로벌 ISP끼리 연결되어야 함
③전 세계의 모든 도시에 존재하는 ISP는 없음 ▷ regional ISP (tier-1 ISP와 연결됨)
  tier-1 ISP는 계층구조의 최상위에 있기 때문에 아무에게도 요금을 지불하지 않음
  접속 ISP – 지방 ISP – 국가 ISP – tier-1 ISP ▷ 네트워크 구조3
④네트워크 구조4 = 네트워크 구조3 + PoP + 멀티홈 + 피어링 + IXP
  PoP(points of presence):제공자의 네트워크 내에 있는 라우터 그룹
  멀티홈: 모든 ISP는 둘 이상의 제공자 ISP에 연결하도록 선택할 수 있음
  피어링: 같은 계층에 있는 가까운 ISP들이 서로 직접 연결하여 상위 계층 ISP를 통하지 않고
          직접 송수신함 ▷ 서로 요금 지불 X
  IXP: 다중의 ISP들이 서로 피어링할 수 있는 만남의 장소
⑤네트워크 구조5 = 네트워크 구조4 + Content-provider network(ex..google,MS,Akamai)
  구글 데이터 센터의 사설 네트워크 서버 = 구글 서버로 오가는 트래픽만을 전달
  tier-1 ISP ex) level 3, sprint, AT&T, NTT
  cp network ex) google => 하위 계층 ISP들과 피어링함으로써 인터넷의 상위 계층을 우회
...
①노드 처리 지연: 패킷 헤더를 조사하고 어디로 보낼지를 결정하는 시간
               ▷라우터는 그 패킷을 라우터 B에 이르는 링크의 앞선 큐에 보낸다
②큐잉 지연: 패킷이 큐에서 링크로 전송되기를 기다리는 시간
            ▷큐에 저장되어 링크로 전송되기를 기다리는 다른 패킷의 수에 의해 결정
            큐잉 지연 = 0 = 큐가 비었으며 다른 패킷이 전송 중인 상태가 아님
③전송 지연: 패킷의 길이가 L비트이고 링크의 전송률이 Lbps라면 L/R
            ▷패킷의 모든 비트를 링크로 밀어내는 데 필요한 시간
④전파 지연: 링크의 처음부터 라우터 B까지의 전파에 필요한 시간
            ▷비트는 링크의 전파속도로 전파됨(거의 빛의속도)
            ▷전파 지연= 두 라우터 사이의 거리(d) / 전파속도(s)
⑤전체 노드 지연 = ①+②+③+④
1.4.2 큐잉 지연과 패킷 손실
큐잉 지연은 패킷마다 다를 수 있음
...
If, 큐가 무한대 비트 저장 가능 ▷ 트래픽 강도(La/R)는 큐잉 지연의 정도 측정에 매우 중요
La/R > 1 = 큐는 끝없이 증가하고 큐잉 지연은 무한대에 도달
La/R <= 1: 도착 트래픽의 특성에 따름
①패킷이 주기적(L/R초마다)으로 도착 = 큐잉 지연 0
②패킷이 Bust = n번째 전송된 패킷은 (n-1)L/R초의 큐잉 지연
실제로는 큐가 무한대 패킷을 가질 수 없다 ▷ 트래픽 강도가 1에 접근함에 따라 패킷 지연이 무한대가 되지 않음 ▷ 큐가 패킷으로 차게됨 ▷ 패킷을 drop or lost
트래픽 강도↑=손실 패킷↑ But, 종단간에 재전송될 수 있음
...
1.4.3 종단간 지연
종단간의 N-1개의 라우터가 있다면
종단간의 지연 = N(처리지연+전송지연(L/R)+전파지연)
<Traceroute> 3N개의 패킷을 목적지로 보냄 n/라우터의 이름/라우터 주소/3번 왕복 지연 시간
1.4.4 컴퓨터 네트워크에서의 처리율(Throughput)
 순간적인 처리율: 호스트 B가 파일을 수신하는 비율(비트/초)
 평균 처리율: 파일이 F비트이고 호스트가 F비트를 수신하는 데 T초가 걸린다면 F/T(비트/초)
 처리율 = 병목 링크의 전송률 = min[Rc,Rs]
...
①계층 내부에서 어떤 동작을 수행
②직접 하위계층의 서비스를 사용
소프트웨어,하드웨어 또는 둘의 통합으로 구현
개념과 구조 측면에서 이점, 모듈화는 시스템 구성요소의 갱신을 더 쉽게 함
단점: 계층의 기능이 하위 계층과 기능적으로 중복
     어느 한 계층에서의 기능이 다른 계층에만 존재하는 정보를 필요로 할 수 있음
프로토콜 스택 = 다양한 계층의 프로토콜을 모두 합한 것
<The Open System Interconnection model>
Application-Presentation-Session-Transport-Network-Data link-Physical
애플리케이션 계층: 네트워크 app과 app 계층 프로토콜이 있는 곳
네트워크 app: 도메인 네임 서버(DNS)가 www.~를 32비트 네트워크 주소로 변환
app 계층 프로토콜: 여러 종단 시스템에 분산되어 있어서, 각 종단 시스템에 있는
            app과 정보 패킷을 교환하는데 사용, 정보 패킷 = 메시지 ex) HTTP, SMTP, FTP
트랜스포트 계층: 클라이언트와 서버 간에 app 계층 메시지를 전송하는 서비스 제공 UDP, TCP
TCP: 연결지향형 서비스 제공 = 전달 보장, 흐름제어, 혼잡제어 기능 제공
UDP: 비연결형 서비스 제공 = 신뢰성, 흐름제어, 혼잡제어 X
트랜스포트 계층 패킷 = 세그먼트
네트워크 계층: 한 호스트에서 다른 호스트로 데이터그램을 라우팅하는 책임을 진다
              ▷ 목적지 호스트의 트랜스포트 계층으로 세그먼트를 운반하는 서비스 제공
①IP 데이터그램의 필드 정의하는 프로토콜을 갖고 있음 = IP 프로토콜
②종단시스템과 라우터가 이 필드에 어떻게 동작하는지를 정의하는 프로토콜을 갖고 있음
데이터그램이 이동하는 경로를 결정하는 라우팅 프로토콜을 포함
 링크 계층
 : 네트워크 계층에서 받은 데이터그램을 경로상의 다음 노드에 전달, 다음 노드에서
 링크 계층은 그 데이터그램을 상위 네트워크 계층으로 보냄
 전체 프레임을 한 네트워크 요소에서 이웃 네트워크 요소로 이동하는 것
 링크 계층 프로토콜 ex) 이더넷, 와이파이, 케이블 네트워크의 DOCSIS 프로토콜, PPP
 데이터그램은 경로상의 서로 다른 링크에서 다른 링크 계층 프로토콜에 의해 처리될 수 있음
 네트워크 계층은 서로 다른 링크 계층 프로토콜로부터 서비스를 제공받음
 링크 계층 패킷 = 프레임(frame)
 물리 계층: 프레임 내부의 각 비트를 한 노드에서 다음 노드로 이동하는 것
           ▷물리 계층 프로토콜은 링크에 의존하고 링크의 실제 전송 매체에 의존
 프레젠테이션 계층: 통신하는 app들이 교환되는 데이터의 의미를 해석하는 서비스 제공
                  ▷데이터 기술(저장되는 내부 포맷 걱정X) + 데이터 압축 + 데이터 암호화
 세션 계층: 데이터 교환의 경계와 동기화를 제공(checking point, recovery of data exchange)
 1.5.2 캡슐화
...
봇넷: 면역되지 않은 장치들로 구성된 네트워크
스파이웨어 맬웨어: keystroke을 다 기록할 수 있어서 해킹 가능
▷ 호스트는 봇넷에 등록될 수 있으며, 스팸 전자메일 분배 혹은 D-DoS 공격을 당할 수 있음
맬웨어는 자기복제를 함 = 호스트의 엔트리를 타고 들어가서 영향을 끼침
바이러스: 사용자와의 상호작용이 필요한 맬웨어
웜: 사용자의 직접적인 상호작용 없이 장치에 침투하는 맬웨어
DoS(Denial of Service): 네트워크, 호스트 혹은 다른 기반구조의 요소들을 정상적인 사용자들이
                      사용할 수 없게 하는 것
①취약성 공격 = 공격받기 쉬운 app이나 OS에 메시지를 보냄
②bandwidth flooding = 수많은 패킷을 보내서 동작 못하게 함
 ▷ 단일 공격 소스라면 충분한 트래픽을 발생 못함
 ▷ 다중의 소스를 제어하여 트래픽을 발생
 ▷ 봇넷을 이용하여 수천 개의 호스트로 트래픽 발생시키는 것이 DDoS
③connection flooding = Half or Fully open된 TCP 연결을 설정 = 가짜 연결을 처리
패킷 스니터 = 지나가는 모든 패킷의 사본을 기록하는 수동적인 수신자
▷ 유선 환경에서도 배치 될 수 있음
▷ 수동적(채널에 패킷을 삽입하지 않음)이기에 탐지하기 어려움
IP spoofing: 거짓의 출발지 주소를 가진 패킷을 인터넷으로 보내는 능력
How to solve? ▷ 종단 인증
